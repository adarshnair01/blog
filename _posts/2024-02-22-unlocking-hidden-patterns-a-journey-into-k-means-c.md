...
2.  **Assignment Step:** For every data point, we calculate its distance to each of the 'K' centroids. The point is then assigned to the cluster whose centroid is closest. We typically use the **Euclidean distance** for this, which for two points $ \mathbf{p}=(p*1, ..., p_n) $ and $ \mathbf{q}=(q_1, ..., q_n) $ is:
    $ d(\mathbf{p},\mathbf{q}) = \sqrt{\sum*{i=1}^{n}(p_i - q_i)^2} $
...
Imagine friends trying to decide where to meet for lunch. Everyone picks a few restaurants (initial centroids). Each person then goes to their _closest_ chosen restaurant (assignment). After that, the "meeting point" for each restaurant group is recalculated based on where everyone who chose it actually lives (update centroid). They repeat until no one wants to switch!
