...
- **Covariance** tells us how two variables vary _together_.
...
- **Eigenvalues:** Each eigenvector has a corresponding eigenvalue. The eigenvalue tells us the magnitude of "stretch" or, more relevantly for PCA, the _amount of variance_ captured along that eigenvector's direction. A larger eigenvalue means that its corresponding eigenvector captures more variance from the data.
...
3.  **Interpretability:** The new principal components are linear combinations of the original features. PC1 might be `0.3*Age + 0.7*Income - 0.2*Education`. While mathematically sound, interpreting what "PC1" _means_ in real-world terms can sometimes be challenging, especially if you retain many components.
4.  **Information Loss:** By reducing dimensionality, you _are_ discarding some information. The goal is to discard the least important information (noise or redundancy) while retaining the signal. Always check the explained variance to ensure you're not losing too much.
