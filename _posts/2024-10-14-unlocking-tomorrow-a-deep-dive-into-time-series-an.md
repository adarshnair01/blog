...
What makes time series special? Unlike standard datasets where each observation is often assumed to be independent, in a time series, _the order matters_. The value today is often influenced by the value yesterday, or last week, or even last year. This dependency is both the challenge and the magic of time series analysis.
...
- **Partial Autocorrelation (PACF):** Measures the correlation between a time series and a lagged version of itself, _after removing the effects of the intermediate lags_. For example, PACF at lag 2 shows the correlation between $Y_t$ and $Y_{t-2}$ that isn't explained by $Y_{t-1}$.
