...
But then, a question started nagging at me. What about data that _doesn't_ fit neatly into a grid or a sequence? What about the intricate web of friendships on social media, the complex structure of a molecule, the interconnected routes of a city's transport system, or even the vast network of scientific papers citing each other? This isn't just a list of items; it's a universe of _relationships_.
...
- **GraphSAGE:** (SAmple and aggreGatE) A highly scalable GNN that learns a function to aggregate information from a _sampled_ set of neighbors, rather than all of them. This is crucial for very large graphs.
