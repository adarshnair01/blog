...
The goal is to choose a split that results in child nodes with lower Gini impurity compared to the parent node. We calculate the Gini impurity for each potential split and pick the one that yields the greatest _reduction_ in impurity. This reduction is sometimes called **Gini Gain**.
