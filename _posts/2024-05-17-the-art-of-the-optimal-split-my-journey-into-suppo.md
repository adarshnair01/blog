...
But here's the kicker: there could be _many_ such lines. So, which one is the "best"?

The genius of SVMs lies in defining "best." They don't just find _any_ separating line; they find the one that has the largest possible distance to the nearest training data point of any class. This distance is called the **margin**.
...
Imagine you have data points forming two concentric circles. No straight line can separate them. The kernel trick suggests: what if we project our data into a higher-dimensional space where it _is_ linearly separable?
...
The "trick" is that we don't actually need to compute these high-dimensional coordinates explicitly. Instead, we use **kernel functions** that calculate the dot product of the data points _as if_ they were already in that higher-dimensional space.
...
- If $0 < \xi_i < 1$, the point is correctly classified but falls _within_ the margin.
