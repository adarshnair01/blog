...
- **How it Works**:
  1.  **Bootstrapping**: We create multiple subsets of the original training data by _sampling with replacement_. This means some data points might appear multiple times in a subset, while others might not appear at all. Each subset is roughly the same size as the original dataset.
...
- **How it Works**:
  1.  **Sequential Training**: Boosting trains base learners _sequentially_. Each new learner is built to specifically correct the errors made by the previous ones.
...
- **How it Works**:
  ...
  2.  **Generate New Features**: Use the _predictions_ of these Level 0 models as input features for a new model. This is where cross-validation typically comes in to prevent data leakage – we train Level 0 models on folds of the data and predict on the out-of-fold data to generate these "meta-features."
...
- **Interpretability**: It can be harder to explain _why_ an ensemble made a particular prediction compared to a single, simpler model.
