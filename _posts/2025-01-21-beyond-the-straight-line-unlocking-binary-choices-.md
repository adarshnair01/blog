...
So far, we know _how_ Logistic Regression makes predictions once it has the weights $\mathbf{w}$. But how do we find the best $\mathbf{w}$? This is the core of any machine learning algorithm: **learning from data**.
...
$$ J(\mathbf{w}) = -\frac{1}{m} \sum\_{i=1}^m [y^{(i)}\log(\hat{y}^{(i)}) + (1-y^{(i)})\log(1-\hat{y}^{(i)})] $$
...
$$ \frac{\partial J}{\partial w*j} = \frac{1}{m} \sum*{i=1}^m (\hat{y}^{(i)} - y^{(i)})x_j^{(i)} $$
