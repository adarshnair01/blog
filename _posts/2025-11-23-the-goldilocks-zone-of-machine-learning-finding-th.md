...
Visually, imagine plotting data points and then drawing a ridiculously wiggly line that perfectly passes through _every single one_ of them, even the obvious outliers. That line is capturing the noise, not the true underlying trend.
...
2.  At some point, the test error stops decreasing and starts to _increase_ even as the training error continues to fall. This is the point where the model starts overfitting.
